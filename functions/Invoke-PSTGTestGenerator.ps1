   ...
   .PARAMETER SqlInstance
    ...
    process {
        ...
        if (-not $SkipProcedureTests) {
            ...
            $dbObjects += Get-DbaModule -SqlInstance $SqlInstance -SqlCredential $SqlCredential -Database $Database -Type StoredProcedure -ExcludeSystemObjects | Where-Object Schema -NotIn $SkipDBTests.name | Select-Object SchemaName, Name
        ...
        if (-not $SkipIndexTests) {
            ...
            if ($Index) {
                $indObjects += $dbObjects.Indexes | Where-Object Name -in $Index | Select-Object Name | Where-Object Schema -NotIn $SkipDBTests.name
            }
            else {
                $indObjects += $dbObjects.Indexes | Select-Object Name | Where-Object Schema -NotIn $SkipDBTests.name
            }
