function New-PSTGProcedureParameterTest {
    ...
    process {
        ...
        if ($InputObject) {
            $objects += Get-DbaModule -SqlInstance $SqlInstance -Database $Database -Type StoredProcedure -ExcludeSystemObjects | Where-Object Name -in $InputObject | Where-Object Schema -NotIn $SkipDBTests.name | Select-Object SchemaName, Name
        }
        else {
            $objects += Get-DbaModule -SqlInstance $SqlInstance -Database $Database -Type StoredProcedure -ExcludeSystemObjects | Where-Object Schema -NotIn $SkipDBTests.name | Select-Object SchemaName, Name
        }

        if ($Schema) {
            $objects = $objects | Where-Object SchemaName -in $Schema | Where-Object Schema -NotIn $SkipDBTests.name
        }

        if ($Procedure) {
            $objects = $objects | Where-Object Name -in $Procedure | Where-Object Schema -NotIn $SkipDBTests.name
        }
